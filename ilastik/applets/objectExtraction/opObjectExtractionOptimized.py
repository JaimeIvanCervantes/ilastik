...
#         self._opRegFeatsAdaptOutput.Input.connect(self._opRegFeats.Output)
        ...
        self._opRegFeatsAdaptOutput.Input.connect(self._opRegionFeatures.Output)
...
#         self.BlockwiseRegionFeatures.connect(self._opRegFeats.Output)
        ...
        self.BlockwiseRegionFeatures.connect(self._opRegionFeatures.Output)
...
#        assert slot == self.LabelImageCacheInput, "Invalid slot for setInSlot(): {}".format(slot.name)
        # Nothing to do here.
...
#   
#         def _do_4d(image, labels, features, axes):
...
#           
#         def compute_global(image, labels, features, axes):
...
#             local = [x+local_suffix for x in local_features]
#             features = list(set(features) - set(local))
#                   
#             return _do_4d(image, labels, features, axes)
#   
#         class Axes(object):
#             x = image.axistags.index('x')
#             y = image.axistags.index('y')
...
#                 selected_vigra_features = feature_dict.keys()
#                 feature_dict.update(default_features)
#                 global_features[plugin_name] = compute_global(image, labels, feature_dict, axes)
#   
